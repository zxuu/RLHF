...
def compute_value_loss(values, old_values, returns, action_mask=None, clip_eps: float = None):
    '''
    # 价值模型的损失使用MSE均方误差，即当前步的预期收益values和真实收益returns（回报）之间的差异。
...
def compute_approx_kl(log_probs: torch.Tensor,ref_log_probs: torch.Tensor,action_mask: Optional[torch.Tensor] = None,):
...
def generate_samples(prompts, model, max_length, max_new_tokens, n_samples_per_prompt, micro_rollout_batch_size):
    ...
    for i in range(0, len(all_prompts), micro_rollout_batch_size):
        ...
        samples = Samples(
            seqs=seqs,    # (batch_size, seq_len) prompt+response的长度
            attention_mask=attention_mask,    # prompt+response的mask
            action_mask=action_mask,    # response的mask
            num_actions=action_mask.size(1), # response的长度
            packed_seq_lens=None,
            response_length=action_mask.float().sum(dim=-1),    # response的长度
            total_length=attention_mask.float().sum(dim=-1),    # prompt+response的长度
        )
...
def compute_rewards(kl, r, action_mask, kl_ctl, clip_reward_value):
    """
    # 返回计算得到的奖励张量，其形状为 (batch_size, seq_len)
    """

    kl_divergence_estimate = -kl_ctl * kl  # kl_ctl是KL控制系数(0.1)
    rewards = kl_divergence_estimate    # 初始的reward就是KL散度

    ends = action_mask.sum(1) + 1    #  记录responses的ends位置
    
    if not isinstance(clip_reward_value, torch.Tensor):
        clip_reward_value = torch.tensor(clip_reward_value).to(r.device)

    reward_clip = torch.clamp(r, -clip_reward_value, clip_reward_value) # 将 r 中的所有元素限制在 [-clip_reward_value, clip_reward_value] 范围内
    batch_size = r.size(0)
    # 遍历每个样本
    for j in range(batch_size):
        # 在每个样本的响应序列结束位置添加裁剪后的奖励值
...
def generate_experiences(samples_list):
    '''
    samples = Samples(
        seqs=seqs,    # shape：(batch_size, seq_len) prompt+response的长度
        attention_mask=attention_mask,    # prompt+response的mask
        action_mask=action_mask,    # response的mask
        num_actions=action_mask.size(1), # response的长度
        packed_seq_lens=None,
    ...
    for samples in samples_list:
        ...
        with torch.no_grad():
            ...
            # 转换成文本，奖励模型是另一个模型，词表不一样，需要转换成文本
            ...
            # 计算kl散度
            # log_probs.float() - ref_log_probs.float()
...
def train_step(experience, steps):
    ...
    '''
    获取当前策略的概率分布 ：
        - 这是为了计算当前策略（更新后的模型）对每个token的预测概率
        - 与之前 old_action_log_probs （旧策略的概率）形成对比
    PPO的核心机制 ：
        - PPO需要比较新旧策略的差异（通过概率比ratio）
        - 旧概率是生成样本时固定的参考点（ .detach() 过的）
        - 新概率是模型参数更新后重新计算的
    '''
